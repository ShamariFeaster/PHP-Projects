<?php
$holder = file_get_contents("screen name logs\\dummy.txt");
...
foreach($screen_names as $screen_name){ //foreach screen name
	...
	for($i=0;$i<count($xml->entry);$i++){ // foreach tweet
		...
		$counter = 1;
		$tcounter = 0;
		foreach($time_array as $time){ // foreach
			$flag = 0;
...
echo "Your analysis of <strong>".count($screen_names)."</strong> profiles and <strong>".$cout."</strong> tweets  has yielded <strong>".$conversation_count.
		"</strong> instances where the user would have seen our ad.<br />This is a success rate of ".round($success,2)."%";
$results = "\r\nYour analysis of ".count($screen_names)." profiles and ".$cout." tweets  has yielded ".$conversation_count.
